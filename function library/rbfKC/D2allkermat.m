...
mat=zeros(nx,ny,dx+nchoosek(dx,2));
for dim=1:dx
    mat(:,:,dim)=4*fmat2.*(repmat(X(:,dim),1,ny)-repmat(Y(:,dim)',nx,1)).^2+2*fmat1;
end

for dim=dx+1:dx+nchoosek(dx,2)-1
    mat(:,:,dim)=4*fmat2.*(repmat(X(:,dim-dx),1,ny)-repmat(Y(:,dim-dx)',nx,1)).*(repmat(X(:,dim+1-dx),1,ny)-repmat(Y(:,dim+1-dx)',nx,1));
end

mat(:,:,dx+nchoosek(dx,2))=4*fmat2.*(repmat(X(:,dx),1,ny)-repmat(Y(:,dx)',nx,1)).*(repmat(X(:,1),1,ny)-repmat(Y(:,1)',nx,1));