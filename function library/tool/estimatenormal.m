...
clear;
close;
load('bunny_15685.mat')
plot3(dsites(:,1),dsites(:,2),dsites(:,3),'r.')
normals = vnormals(dsites, faces);
...
[indices, distances] = knnsearch(Mdl, dsites, 'K', 100); % 包括点本身
ndsites = size(dsites,1);
normal_loc = zeros(size(dsites));
...
t = faces;
normals = normal_loc;
save('bunny_15685.mat','p','t','normals')
