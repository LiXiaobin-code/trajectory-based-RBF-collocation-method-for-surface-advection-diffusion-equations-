...
set(h1, 'Position', [0.9 0.25 0.03 0.5]);  % Adjust colorbar position
% print('-dpng', '-r400', 'CN_Torus'); % Save the figure as a PNG file with 400 DPI

frame_file = sprintf('%s/CN_Torus.png', output_folder);
...
set(gca, 'Position', [0.05 0.05 0.85 0.9]);  % Adjust plot area to leave space for colorbar
set(h1, 'Position', [0.9 0.25 0.03 0.5]);  % Adjust colorbar position
frame_file = sprintf('%s/SL_Torus.png', output_folder);
exportgraphics(fig, frame_file, 'Resolution', 200);

fig = figure(3);

 load('TCNtorus.mat')
h=trisurf(t,p(:,1),p(:,2),p(:,3),abs(u-u0));
colormap(jet)
h1 = colorbar;
h1.Ruler.Exponent = -2;
...
frame_file = sprintf('%s/CN_Torus_error.png', output_folder);
exportgraphics(fig, frame_file, 'Resolution', 200);
fig = figure(4);
...
frame_file = sprintf('%s/SL_Torus_error.png', output_folder);
exportgraphics(fig, frame_file, 'Resolution', 200);
