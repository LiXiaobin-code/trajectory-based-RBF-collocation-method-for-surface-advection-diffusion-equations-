...
% 绘制不同 dt 的曲线
for eps_idx = 1:length(dts)
    dt = dts(eps_idx);
    filename = sprintf('tCN_dt_%g', dt);
    load(filename); % 加载 tCN
    
    tCN_sampled = tCN(1:k:end); % 采样后的 tCN
    
    plot(theta_sampled, tCN_sampled, ...
        'Color', colors(eps_idx, :), ...
        'LineStyle', line_styles{eps_idx}, ...
        'Marker', markers{eps_idx}, ...
        'MarkerSize', 3, ...
        'LineWidth', line_widths(eps_idx), ...
        'DisplayName', sprintf('\\Delta t = %.0e', dt));
end
...
set(gca, 'XTick', 0:pi/2:2*pi, ...
         ...
         'FontSize', 12);
% grid on
print(gcf, '-dpng', '-r600', 'tCN_dt_lines.png');